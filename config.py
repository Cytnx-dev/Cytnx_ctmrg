...
def configure(parsed_args):
    ...
    #     assert global_args.gpu and cytnx.cuda.device(global_args.gpu), "CTMARGS_step_core_gpu"\
...
def print_config():
    ...
    print(f"Pycytnx: {cytnx.__version__}")
    try:
        import scipy
    ...
    try:
        ...
        print(f"peps-cytnx git ref: {ret.stdout.rstrip()}")
    except subprocess.CalledProcessError as e:
        print(f"peps-cytnx git ref: N/A")
    try:
        import subprocess
        import pathlib
        root_dir= pathlib.Path(__file__).parent.resolve()
...
class GLOBALARGS():
    ...
    :ivar dtype: data type of all cytnx.tensor. Default: ``float64``
    :vartype dtype: cytnx.dtype
    :ivar device: device on which all the cytnx.tensors are stored. Default: ``'cpu'``
    :vartype device: str
...
class CTMARGS():
    ...
    :ivar projector_svd_method: singular/eigen value decomposition algorithm used in the construction
                                of the projectors:
                                    ...
                                    * ``'GESDD'``: pycytnx wrapper of LAPACK's gesdd
                                    * ``'RSVD'``: randomized SVD
                                    * ``'SYMEIG'``: pycytnx wrapper of LAPACK's dsyev for symmetric matrices
...
class OPTARGS():
    ...
    :ivar tolerance_grad: stopping criterion wrt. norm of the gradient (which norm ? See 
                          ``cytnx.optim.LBFGS``). Default: ``1.0e-5``
